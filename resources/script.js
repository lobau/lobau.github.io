...
class ShadedBox {
	...
	updateShadow(e) {
		...
		var maxShadows = 3;
		var shadowString = "";
		// shadowString += `inset 0px 0px 2px 1px rgba(0, 0, 0, 0.1), `;
		...
		// shadowString += `, inset ${innerShadowOffsetX}px ${innerShadowOffsetY}px 10px -3px rgba(255, 255, 255, 0.35)`;
...
var shadedElements = document.getElementsByClassName("shaded");
var shadedBoxes = [];
for (var i = 0; i < shadedElements.length; i++) {
	shadedBoxes.push(new ShadedBox(shadedElements[i]));
}
